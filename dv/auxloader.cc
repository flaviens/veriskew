...
// main dimension: aux id.
// pair <word address, content>
// std::vector<std::vector<std::pair<uint64_t, std::vector<uint8_t>>>> aux_assignments;

// Zeroth dim: aux id.
// First  dim: absolute word address.
// Second dim: content.
std::vector<std::unordered_map<size_t, std::vector<uint8_t>>> word_aux_contents;
static size_t glob_word_width_bytes = -1;

extern "C" {
  void init_aux_vectors(long long num_auxs);
  char read_aux_section(long long aux_id, long int *word_address, const svOpenArrayHandle svbuffer);
  void read_auxs(const char* filename, int word_width_bytes);
}

extern "C" void init_aux_vectors(long long num_auxs) {
  word_aux_contents.resize(num_auxs);
}
...
static inline char char_to_hex(char c) {
  ...
  std::cerr << "Error in aux assignment string. Unexpected character: `" << c << "`" << std::endl;
  exit(0);
}

// @return 1 if this is a valid response, 0 if there is nothing anymore to supply, and the outputs should be ignored.
extern "C" char get_next_aux_word(long long aux_id, long int *word_address, const svOpenArrayHandle svbuffer) {
  static auto itr = word_aux_contents[aux_id].begin();

  // If the end of the structure was reached.
  if (itr == word_aux_contents[aux_id].end()) {
    // Set it back to the beginning so another mem can read it as well (used in Ibex).
    itr = word_aux_contents[aux_id].begin();
    return 0;
  }
  ...
  *word_address = itr->first;
  ...
  char* buf = (char*)svGetArrayPtr(svbuffer);
...
extern "C" void read_auxs(const char* filename, int word_width_bytes) {
  ...
  std::string byte_aux_contents_str;
  ...
  while (std::getline(f, tmp_line)) {
    ...
    isstream >> byte_aux_contents_str;
    ...
    if (byte_aux_contents_str.size() < 2*num_bytes) {
    ...
    for (size_t curr_byte_addr = base_addr; curr_byte_addr < base_addr+num_bytes; curr_byte_addr ++) {
      ...
      uint8_t byte_val = (char_to_hex(byte_aux_contents_str[2*byte_offset]) << 4) | char_to_hex(byte_aux_contents_str[2*byte_offset+1]); 

      // If the word is not yet in the new words struct, then add it.
      if (word_aux_contents[aux_id].find(curr_abs_word_addr) == word_aux_contents[aux_id].end()) {
        std::vector<uint8_t> new_word_content(word_width_bytes, 0);
        word_aux_contents[aux_id].insert({curr_abs_word_addr, new_word_content});
      }

      // Add the content to the word
      size_t byte_id_in_word = curr_byte_addr % word_width_bytes;
      word_aux_contents[aux_id][curr_abs_word_addr][byte_id_in_word] = byte_val;
    }
  }
